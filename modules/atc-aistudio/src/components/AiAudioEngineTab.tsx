...
import { Music, Loader2, StopCircle, Code, Play } from 'lucide-react';

interface AiAudioEngineTabProps { selectedFranchise: any; onAssetGenerated?: (tag: string, name: string, desc: string) => void; }
export function AiAudioEngineTab({ selectedFranchise, onAssetGenerated }: AiAudioEngineTabProps) {
  const [topic, setTopic] = useState('');
  const [flavor, setFlavor] = useState('Web Audio API Synth');
  ...
  const handleGenerateAudio = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/franchise-factory/generate-audio', {
        method: 'POST',
      ...
      if (response.body) {
        ...
        if (htmlBuffer && onAssetGenerated) {
          onAssetGenerated('Audio', `Audio: ${topic.substring(0, 30)}...`, htmlBuffer.replace(/^```html\n?/, '').replace(/```$/, ''));
        }
      }
    } catch (err: any) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-250px)] min-h-[700px]">
      <div className="lg:col-span-4 p-6 rounded-2xl bg-[#0b0f19] border border-cyan-500/20 shadow-[0_0_20px_rgba(6,182,212,0.05)] flex flex-col gap-6 overflow-y-auto custom-scrollbar">
        <div className="flex items-center gap-3 border-b border-white/10 pb-4">
          ...
          <h3 className="text-lg font-semibold text-white">Audio KI Engine</h3>
        </div>

        <div className="flex flex-col gap-4">
          <div>
            <label className="block text-xs font-mono uppercase tracking-wider text-slate-400 mb-2">Beschreibung des Sounds</label>
            <textarea
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
              placeholder="Z.b. Ein prozeduraler Ambient-Track für eine Cyberpunk-Lounge..."
          ...
          <div>
            <label className="block text-xs font-mono uppercase tracking-wider text-slate-400 mb-2">Technik</label>
            <select
              value={flavor}
              ...
              disabled={isGenerating}
            >
              <option value="Web Audio API (Native)">Native Web Audio API</option>
              <option value="Tone.js via CDN">Tone.js (Synthesizer & Effects)</option>
              <option value="8-Bit Chiptune">8-Bit Chiptune</option>
              <option value="Procedural Ambient">Procedural Ambient</option>
            </select>
          </div>
      ...
      <div className="lg:col-span-8 p-1 rounded-2xl bg-black border border-white/10 flex flex-col relative overflow-hidden group">
        <div className="absolute top-4 right-4 z-10 flex border border-white/20 bg-black/80 rounded-lg shadow-lg overflow-hidden backdrop-blur-md opacity-0 group-hover:opacity-100 transition-opacity">
           <button onClick={() => setViewMode('preview')} className={`px-4 py-2 text-xs font-mono font-bold tracking-wider uppercase transition-colors flex items-center gap-2 ${viewMode === 'preview' ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-400 hover:text-white'}`}><Play className="w-3 h-3" /> Player</button>
          ...
          <div className="flex-1 w-full h-full bg-[#0b0f19] rounded-xl overflow-hidden relative">
            {generatedHtml ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-600 font-light gap-3 text-center">
                <Music className="w-16 h-16 opacity-10 mb-4" />
                <p>Der Audio Player ist leer.</p>
                <p className="text-sm">Beschreibe deine Vision und lass die KI einen prozeduralen Web-Synthesizer bauen.</p>
              </div>
            )}
            {isGenerating && (
              <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center text-cyan-400 font-mono text-sm z-20">
                ...
                <div>Generiere Audio Engine Code...</div>
              </div>
            )}
          </div>
        ) : (
