...
import { Film, Loader2, StopCircle, Code, Play } from 'lucide-react';

interface AiAnimationEngineTabProps { selectedFranchise: any; onAssetGenerated?: (tag: string, name: string, desc: string) => void; }
export function AiAnimationEngineTab({ selectedFranchise, onAssetGenerated }: AiAnimationEngineTabProps) {
  const [topic, setTopic] = useState('');
  const [flavor, setFlavor] = useState('CSS Keyframes');
  ...
  const handleGenerateAnimation = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/franchise-factory/generate-animation', {
        method: 'POST',
      ...
      if (response.body) {
        ...
        if (htmlBuffer && onAssetGenerated) {
          onAssetGenerated('Animation', `Animation: ${topic.substring(0, 30)}...`, htmlBuffer.replace(/^```html\n?/, '').replace(/```$/, ''));
        }
      }
    } catch (err: any) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-250px)] min-h-[700px]">
      <div className="lg:col-span-4 p-6 rounded-2xl bg-[#0b0f19] border border-cyan-500/20 shadow-[0_0_20px_rgba(6,182,212,0.05)] flex flex-col gap-6 overflow-y-auto custom-scrollbar">
        <div className="flex items-center gap-3 border-b border-white/10 pb-4">
          ...
          <h3 className="text-lg font-semibold text-white">Animation KI Engine</h3>
        </div>

        <div className="flex flex-col gap-4">
          <div>
            <label className="block text-xs font-mono uppercase tracking-wider text-slate-400 mb-2">Beschreibung der Animation</label>
            <textarea
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
              placeholder="Z.b. Eine explodierende Neon-Nova mit Partikeln..."
          ...
          <div>
            <label className="block text-xs font-mono uppercase tracking-wider text-slate-400 mb-2">Technologie / Variante</label>
            <select
              value={flavor}
              ...
              disabled={isGenerating}
            >
              <option value="CSS Keyframes">CSS Keyframes</option>
              <option value="HTML5 Canvas / RequestAnimationFrame">HTML5 Canvas</option>
              <option value="GSAP / JS Animation">GSAP / JS Animation</option>
              <option value="SVG Animation">SVG SMIL/CSS</option>
            </select>
          </div>
          ...
          <div className="flex-1 w-full h-full bg-[#0b0f19] rounded-xl overflow-hidden relative">
            {generatedHtml ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-600 font-light gap-3 text-center">
                ...
                <p>Der Animations Canvas ist leer.</p>
                <p className="text-sm">Beschreibe deine Vision und lass die KI den Animationscode schreiben.</p>
              </div>
            )}
            {isGenerating && (
              <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center text-cyan-400 font-mono text-sm z-20">
                ...
                <div>Generiere Animation Code...</div>
              </div>
            )}
          </div>
        ) : (
